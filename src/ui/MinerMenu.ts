...
export class MinerMenu extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    const title = scene.add.text(0, -PANEL_H / 2 + 22, 'Drill', {
    ...
    const hintFuel = scene.add.text(0, -46, '1 battery = 200 hits  •  pickaxe = 2× speed', {
    ...
    // Pickaxe toggle
    this.pickaxeBtnBg = scene.add.rectangle(80, -10, 110, 30, 0x2a4a2a)
      .setStrokeStyle(1, 0x52b788)
      .setInteractive(new Phaser.Geom.Rectangle(-55, -15, 110, 30), Phaser.Geom.Rectangle.Contains);
    this.pickaxeBtnText = scene.add.text(80, -10, 'Ins. Pickaxe', {
    ...
    this.pickaxeBtnBg.on('pointerover', () => {
      const has = this.pickaxeBtnText.text.startsWith('Rem');
      this.pickaxeBtnBg.setFillStyle(has ? 0x5a2a2a : 0x3a6a3a);
    });
    this.pickaxeBtnBg.on('pointerout', () => {
      const has = this.pickaxeBtnText.text.startsWith('Rem');
      this.pickaxeBtnBg.setFillStyle(has ? 0x4a2a2a : 0x2a4a2a);
    });
    ...
    this.takeBtnBg.on('pointerdown', () => scene.game.events.emit('take-miner-output'));
  ...
  refresh(power: number, hasPickaxe: boolean, output: { itemId: string; quantity: number } | null): void {
